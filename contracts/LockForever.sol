...
contract LockForever is ERC20, Ownable, ILockForever {
    ...
    constructor(address _tokenToLock, string memory name_, string memory symbol_, uint8 decimals_) ERC20(name_, symbol_, decimals_) {
    ...
    function lock(address account, uint256 amount) public returns (bool) {
        require(
            !isPaused,
            "Lock Forever: locking has been paused"
        );
        require(
            amount > 0,
            "Lock Forever: amount cannot be 0"
        );
        ...
        bool result = IERC20(tokenToLock).transferFrom(_msgSender(), address(this), amount);
        
        require(
            result,
            "Lock Forever: Token transfer failed"
        );
    ...
    function getWalletLockRecordsLength(address account) public view returns (uint256) {
        uint256 lockRecordsLength = lockRecordIds[account].length;
        return lockRecordsLength;
    }

    function getWalletLockRecords(address account) public view returns (LockRecord[] memory) {
        uint256 lockRecordsLength = lockRecordIds[account].length;
